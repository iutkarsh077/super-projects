...
const Signup = () => {
    ...
    const handleRegisterUser = async () => {
        ...
        try{
            const res = await api.post("/register", formDetails);
            console.log(res)
        }catch(error){
            console.log(error);
        }
    ...
    return (
        <div className="w-screen h-screen overflow-hidden flex justify-center items-center">
            <div className="flex flex-col gap-6 w-[33%] bg-gray-600 h-[33%] p-5 rounded-2xl">
                <input type="text" className="border  border-black rounded-md pl-2 h-10" value={formDetails.name} onChange={(e) => setFormDetails((prev) => ({ ...prev, name: e.target.value }))} placeholder="Enter your name" />
                ...
                <button onClick={handleRegisterUser} className="bg-blue-500 max-w-fit flex p-2 rounded-lg hover:cursor-pointer">Register User</button>
            </div>
        </div>
    )
}
